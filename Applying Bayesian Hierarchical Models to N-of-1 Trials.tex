...
practitioner sees daily in her office (citation?).
...
across time with single patient using different treatments. In this design
...
\begin{def}\label{}
\begin{equation}\label{quadratictimetrend}
...
express the model in terms of of the probability:
...
applies to a simple model\ \ref{oneerror} with only the treatment effects \(\mu \) and a random error \(\epsilon \).

\begin{def}\label{}
\begin{equation}\label{}
P(A|B) = \frac{P(B|A)P(A)}{P(B)},
y_{t} = \mu_m + \epsilon_{t},
\end{equation}
\end{def}where \(\epsilon_{t} \sim N(0,\sigma^2)\). Time \(t\) is indexed from the
beginning of the study.

In this case when we want to estimate the parameters of the different treatment effect \(\mu_m \).
...
and the error term \(\epsilon_t \) so \(y_t \sim N(\mu_1,\sigma^2) \). This is means that the likelihood
term in bayesian formula is a simple product of the probability density functions of
a normal distribution with mean \(\mu_1 \) and variance \(\sigma^2 \), because our model assumes
that the observations are independent (e.g.\ no autocorrelation included):

\begin{def}\label{}
\begin{equation}\label{}
    P(Y_{t_{\mu_1}}|\mu_1, \sigma)
    =
    \prod_{Y_{t_{\mu_1}}}
    \frac{1}
    {{\sqrt {2\pi \sigma^2} }}
...
\end{equation}
\end{def}where during \(t_{\mu_1}\) the treatment of interest was given \(\mu_m=\mu_1\). We assumed
in our model that observations are independent

To avoid the complexity of dealing with multiple unknown variables (\(\mu_1 \) and \(\sigma \)
in this case) we assume that \(\sigma \) is known. Now we have to define only one
prior distribution (we will later come back later to the case of multiple unknown variables).
We choose a normal distribution for the prior of \(\mu_1\) is defined by mean \(\lambda_0 \)
...
has some nice algebraic properties that we will soon see.
...
over possible values of our unknown parameter \(\int P(Y_{t_{\mu_1}}|\mu_1)P(\mu_1) d\mu_1 \),
we can see that the term is not dependent on the specific values of \(\mu_1 \), but just the data.
This means that the term is just a constant and as such does not affect shape of the posterior distribution,
that is \(P(\mu_1|Y_{t_{\mu_1}}) \propto P(Y_{t_{\mu_1}}|\mu_1)P(\mu_1) \). If we look at this more
