...
By first exponentiating\ \ref{bin_exponentiating} and then using simple algebraic manipulation we can
express the model in terms of of the probability \(p_t\)\ \ref{bin_algmanipulation}:

\begin{def}\label{}
\begin{equation}\label{bin_exponentiating}
\frac{p_t}{1-p_t}=e^{\beta_0 + \beta_1 t + \mu_t}
\end{equation}
\end{def}

\begin{def}\label{}
\begin{equation}\label{bin_algmanipulation}
...
effect of the treatment given during time \(t\). By exponentiating\ \ref{ord_exponentiating}
and algebraic manipulation we end up with similar model as with binary outcomes\ \ref{ord_algmanipulation},
...
We could make it more clear what the marginal likelihood \(P(D)\) stands for by
...
applies to the case of model\ \ref{oneerror} with only the treatment effect and a random error.

\begin{def}\label{}
\begin{equation}\label{oneerror_bayes}
y_{mbpt} = \mu_m + \epsilon_{t(p(b))},
\end{equation}
\end{def}where \(\epsilon_{t(p(b))} \sim N(0,\sigma^2_{\epsilon})\).
...
Prior normally distributed

and the likelihood is also normally distributed \(P(D|\Theta) \sim N(\theta, \delta^2)\)
