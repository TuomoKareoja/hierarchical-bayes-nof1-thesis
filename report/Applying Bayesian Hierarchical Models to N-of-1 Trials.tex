...
apply Bayesian methods in estimating the effectiveness of different
...
Knowing the best treatment in average might not help much in finding the right
...
usually is still lots of unexplained variance left after this (TODO source). Some
...
a common practice to accept patients to RCT:s only if they don't suffer from any
...
that have some event of interest. To repeat the same experiment its design
...
this by calculating the distribution of heads for multiple 20 coin toss trials
assuming that the coin is fair. Then we can measure our actual result against this
distribution. If the result seems to fall ``nicely'' within this distribution, then we
can say that the null hypothesis stand and the coin is fair. The problem comes if we did
...
that made us decide to do more coin tosses.
...
individual patient the distribution where they come from represents the population and
its parameters represent the tendency of the population where the patient is drawn from. For example we could assume that the
...
comes from this distribution and the shape of this distribution makes certain values for
patient-level parameters more or less probable.
