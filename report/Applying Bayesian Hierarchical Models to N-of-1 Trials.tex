...
On top are the prior parameter values given to each patient, next the individual patient
parameters are drawn from distributions defined by the prior parameters and at the
bottom individual observations are drawn from a normal distribution defined by
parameters drawn from the patient-specific prior distributions.

To broaden this model from just being a bunch of patient-specific models we can combine
them by adding a population level that links are patients together:
...
Now the previous level where we had the parameters defining the patient level priors we
have a population level distribution where the patient level parameters are drawn from
and we only define priors for the population level.
...
The question now becomes, how we apply the Bayesian formula in this case? Luckily this
happens to be very simple as we just have to remember the probability chain rule that
...
Now just take the basic Bayes formula and instead with three parameters and apply this rule


\begin{align}\label{conjugatebayesrule}
    P(A,B|C) & = \frac{P(C|A,B)P(A \cap B)}{P(A \cap B)} \\
    & = \frac{P(C|A,B)P(B|A)P(A)}{P(A \cap B)} \nonumber \\
    & \propto {P(C|A,B)P(B|A)P(A)} \nonumber \\
\end{align}
...
the value of \(\sigma \) and \(\sigma \) is an independent variable. We also notice that
we could just keep on adding new parameters as could just apply the chain rule again.

Lets now use this knowledge to the example model we defined
above:

\begin{align}\label{conjugatebayesrule}
    P(\theta_{i \dots p},\tau_{i \dots p},\mu,\sigma,\beta| Y) & \propto P(Y|\theta_{i \dots p},\tau_{i \dots p},\mu,\sigma,\beta)P(\theta_{i \dots p} \cap \tau_{i \dots p} \cap \mu \cap \sigma \cap \beta) \\
    & = P(Y|\theta_{i \dots p},\tau_{i \dots p},\mu,\sigma,\beta) P(\theta_i | \theta_{i+1 \dots p} \cap \tau_{i \dots p} \cap \mu \cap \sigma \cap \beta) \nonumber \\
    & P(\theta_{i+1 \dots p} \cap \tau_{i \dots p} \cap \mu \cap \sigma \cap \beta) \nonumber \\
    & \ldots \nonumber \\
    & = P(Y|\theta_{i \dots p},\tau_{i \dots p},\mu,\sigma,\beta) P(\theta_i | \theta_{i+1 \dots p} \cap \tau_{i \dots p} \cap \mu \cap \sigma \cap \beta) \nonumber \\
    & P(\theta_i, \theta_{i+1} | \theta_{i+2 \dots p} \cap \tau_{i \dots p} \cap \mu \cap \sigma \cap \beta) \ldots P(\theta_{i \dots p} \cap \tau_{i \dots p} \cap \mu \cap \sigma \cap | \beta) P(\beta) \nonumber
\end{align}
