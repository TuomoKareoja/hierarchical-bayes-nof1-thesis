...
% We view any noninformative or weakly-informative prior distribution as inherently
% provisional—after the model has been fit, one should look at the posterior distribution
% and see if it makes sense. If the posterior distribution doesnot make sense, this
% implies
% that additional prior knowledge is available that has not been included in the model,
% and that contradicts the assumptions of the prior distribution that has been used.
% It isthen appropriate to go back and alter the prior distributionto be more consistent
% withthis external knowledge.
...
% TODO add an example about how to choose variance prior

% TODO how about using exponential distribution for hierarchical variance priors

% TODO what are prior predictive checks
...
% TODO make sure that the values are reasonablsubsection{Implementing the Model in PyMC3}
...
% TODO check that explanation matches the one in the bayes problems chapter
...
while \pyv{ess_mean} is for the estimation of the mean and
...
% TODO have we mentioned these samplers before?
...
% TODO comment about the other parameters
...
% This needs be cleared up and you need to understand do we have to use a scale
% parameter
...
% TODO How to define the variance from existing data?
...
% TODO make sure we use reasonable priors

% TODO should we have all examples with uninformative priors
...
% TODO check that codes contains the right parameters and all necessary parts
...
still intelligible and see wee that the posteriors of the three chain align even though
...
sure that we don't have a problem. Effective sample size \pyv{ess_*} show that all
...
quantiles \pyv{ess_tail}. Still, we have a lot of
steps and just by brute force we have reasonable effective sample sizes. With
\(\hat{R}\)-statistic (\pyv{r_hat}) we don't see any problems as
...
% TODO would reparametrizing the model help here?
...
% TODO after tuning and parametrizing the model properly, write this out
...
\begin{figure}[H]
    ...
    \bigskip
    \centering
    \includegraphics[width=\textwidth,height=2.8in,keepaspectratio]{posterior_tightness.pdf}
\end{figure}
\bigskip
