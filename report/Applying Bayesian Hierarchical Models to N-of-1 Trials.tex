...
interesting comparison as we can see how the estimate that we get for this parameters
...
Normally before we would start with more extensive exploration of the data, but as teaching preliminary analysis
...
with pm.Model() as single_patient_no_trend_model:
    ...
    # the treatment 1 prior or treatment 2 prior + trend at that that measurement
    # with always the same variance
...
% TODO what is the actual number of steps as the amount of parameters affects this!
...
% TODO add the command line printout for running the model
...
% TODO what does an improperly defined maen
% TODO add an example of traceplots when no tuning is done
...
% TODO check Kruschke for explaining the problem
...
% TODO add timeseries plots of the values to show that simulated datasets actually look
% like new experiments
...
\begin{tikzpicture}
    ...
    \matrix[matrix of math nodes, column sep=-35pt, row sep=30pt] (mat)
    {
        ...
        \ & \theta_i, \eta_i, \beta_i, \tau_i & \\\
        \ \\
        y_{1,1}, \ldots, y_{28, 1} & \ldots & y_{1,6}, \ldots, y_{28, 6} \\
    };

    \draw[->,>=latex] (mat-1-2) -- (mat-2-2);
    \draw[->,>=latex] (mat-2-2) -- (mat-4-1);
    \draw[->,>=latex] (mat-2-2) -- (mat-4-3);

    \node[anchor=east] at ([xshift = 0pt]mat-2-1)
    {$
        \theta_{i} \sim \mathcal{N}(\mu_{\theta},\sigma_{\theta}^2),
        \eta_{i} \sim \mathcal{N}(\mu_{\eta},\sigma_{\eta}^2),
        \beta_{i} \sim \mathcal{N}(\mu_{\beta},\sigma_{\beta}^2),
    $};
    \node[anchor=east] at ([xshift = 0pt]mat-3-1)
    {$
        \tau_{i} \sim HalfCauchy(0, \gamma_{\tau}),
    $};

\end{tikzpicture}

Last we need to define prior distributions for the parameters defining the population distribution:
...
As we are now already familiar with basics of PyMC3, we can now skip explaining some
parts of the code.
