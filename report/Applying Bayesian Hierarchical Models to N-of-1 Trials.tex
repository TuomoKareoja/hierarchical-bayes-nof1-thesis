...
\begin{def}\label{}
    \begin{equation}\label{conjugatebayesrule}
        ...
        % &\text{$\theta$ and $\tau$ are independent on each other and are dependent on different population level parameters} \nonumber \\
...
a hospital are needed to take the measurements. The condition is also usually steadily
...
\begin{itemize}
    ...
    \item The week-long treatment periods will be organized in to two block, each of which will run for
...
actually doing. For simulating each observation \(t\) for patient \(i\) we use the following model:
...
% TODO mention why we don't care a about the baseline
...
that define the distribution of the patient level parameters:
...
\begin{tikzpicture}
    ...
    \matrix[matrix of math nodes, column sep=0pt, row sep=30pt] (mat)
    {
    \mu_{\theta}, \sigma_{\theta}, \mu_{\eta},\sigma_{\eta}, \alpha_{\tau},
    \beta_{\tau}, \alpha_{\rho}, \beta_{\rho} \\
        \theta_1, \eta_1, \epsilon_1, \rho_1 \\
    ...
    \node[anchor=east] at ([xshift = -35pt]mat-2-1)
    {$\theta_i \sim \mathcal{N}(\mu_{\theta},\sigma_{\theta}^2), \eta_i \sim \mathcal{N}(\mu_{\eta},\sigma_{\eta}^2), \tau_i \sim Beta(\alpha_{\tau}, \beta_{\tau}), \rho_i \sim InvGamma(\alpha_{\rho}, \beta_{\rho})$};

\end{tikzpicture}

We set the population-level parameters to the following values and then run the
simulation:

% TODO are we using shape and scale or some other definition? check python fuctions!
...
\begin{align}\label{populationparameters}
    ...
    \text{population measurement error shape: } & \alpha_{\tau} = 3 \nonumber \\
    \text{population measurement error scale: } & \beta_{\tau} = 0.3 \nonumber \\
    \text{population error autocorrelation alpha: } & \alpha_{\rho} = 100 \nonumber \\
    \text{population error autocorrelation beta: } & \beta_{\rho} = 200 \nonumber
\end{align}

Lets first look at the actual measurements. We can clearly see the for the highlighted
patient one treatment is working better than the other one as we can easily pickout the
ABBA treatment assignment from the picture. We can also see that for this patient the t

% TODO choose if up or down is better and make the models work that like this and the
% model definition match this
...
Let's next look at the patient level parameters

% TODO show both treatment A and B in the same graph to make comparisons possible
...
Lastly lets see our defined population distributions
% TODO show both treatment A and B in the same graph to make comparisons possible
