...
patient treatment effect taken from other studies. Lets say that this value is
...
that should be fairly non-informative considering the value range we are dealing with.
The set prior values can be seen in table\ \ref{singlelpatientpriorvalues}.
...
% need to make the code look cleaner without the footnote getting in the way
\pagebreak
...
timelines match with the real timeline. This can be seen from figure\
\ref{singlepatientposteriortimeline}. All 500 samples are drawn as grey lines and the
actual measurement with a red line. We can see that the results line up quite nicely.
...
called ``draw posteriors''. We don't go over the code of this function,
...
% TODO should we use estimates from the data for the variance?
% Are we talking about variance or standard deviation
...
\begin{tikzpicture}
    ...
    \draw[->,>=latex] (mat-1-2) -- (mat-2-2);
    \draw[->,>=latex] (mat-2-2) -- (mat-6-2);
    \draw[->,>=latex] (mat-6-2) -- (mat-8-1);
    \draw[->,>=latex] (mat-6-2) -- (mat-8-3);
    ...
    {$
        \gamma_{\tau} \sim HalfCauchy(0, \gamma_{\gamma_{\tau}})
    $};
    ...
    $};
    \node[anchor=east] at ([xshift = 0pt]mat-7-1)
    {$
        \tau_{j} \sim HalfCauchy(0, \gamma_{\tau}),
    $};
    \node[anchor=east] at ([xshift = -35pt]mat-8-1)
    {$
        y_{tj} = Z_{tj}\theta_{j} + W_{tj}\eta_{j} + t\beta_{j} + \epsilon_{tj}
    $};
\end{tikzpicture}
...
% TODO Why can we slot in a variance from measures in hierarchical models?
...
% TODO population error variance?
...
patient\_idx vector. This vector has a length 168 and its values are the indices of the
...
patient\_idx vector. Note that unlike in the single patient case where we could used a single
...
\(\hat{R}\) (r\_hat) diagnostic metric\ \ref{hierarchicalmodeldiagnosticmeasures}. As
...
\begin{figure}[H]
    ...
    \bigskip
    % TODO after everything else make sure that table fits the page nicely
    \resizebox{!}{3.2in}{%
...
\begin{figure}[H]
    ...
    \label{hierarhicalmodelposteriortimeline}
    \bigskip
...
To get more precision lets use our draw\_posterior\_checks function that we
used in single patient model. Even though the posterior predictions that we now get from
from the PyMC3 inbuilt sample\_posterior\_predictive function, our function is written
...
single patient model and the hierarchical model, for all patients (separate analysis was
...
between treatment noticeably smaller, and tightened the distribution. When looking at
just our main outcome of interest it seems, that the hierarchical model did not make a
...
hierarchy. In figure\ \ref{posteriorshrinkage} we have plotted the estimated mean of
...
treatments tend to move toward the same direction and also the same amount. The
interesting question is what determines how much the estimates change. It is clear that
the model is not just drawing the estimates that are farthest from other observations
...
observations will get drawn more strongly towards others, but this is not the only
factor in play. The other significant thing is how strong the estimates for single
patient are.
...
In figure\ \ref{modeltightness} we have plotted the posterior estimates for the
...
are. Patients 2 and 6 stick out here, with patient 6 having significantly tighter
distribution than other patient and the patient 2 having a very wide distribution. This
...
\bigskip
\begin{figure}[H]
    \caption{Single Patient Model Posterior Estimate Distribution}
    \label{modeltightness}
    \bigskip
...
% TODO list most important points based on chapters
