...
The general outline for the algorithm for solving this problem is as follows:
...
values give us a good estimate of the entire posterior distribution. Why does this work?
The crucial part in the algorithm is the step 3.
...
random value picker works here and we get a proper sample eventually. The problem is
that with a bad algorithm this could take a long long time. What we would want here is
way to create proposed steps that are rejected as little as possible. This leads to
quick movement over posterior distribution and more efficient sampling. Some of the
simpler solutions to this problem are still explainable (e.g. Gibbs sampling), but
...
ball on terrain defined by the inverse of our distribution\ \cite{kruschke}. The
technicalities how this works in practice are not for the faint of heart.

Below in figure\ \ref{traceexample} we have visualization of the 200 first steps taken
by the NUTS sampler when estimating two parameters. We can see that there seem to be a
clear correlation between the parameters and our sample is already reflecting that. We
also see that the sampling is quite clever with the algorithm taking larger steps at
less probable parts of the distribution. This means that algorithm does not get stuck in
the less probable parts of the distribution and thus moves more efficiently.

\begin{figure}[H]
    \centering
    \caption{First 200 Steps When Estimating Two Parameters With the NUTS Sampler}\label{traceexample}
...
all patient-level parameters can already be thought as defining this population level distribution, but the difference is that the prior
is set in stone, but when we make the model hierarchical, the population-level distribution works just like the patient-level
...
patient-level estimates closer together, but here the as the distribution is itself
estimated from the data, this happens only if the data seem to suggest commonalities
...
models as a trees. In the top of the tree we have prior values that define the shape of
prior distribution next level down, from these distributions come the parameters that
define the data creation process at the bottom of the tree. Lets take a simple example where we have $p$ patients with $n$
...
% TODO need to make the code look cleaner without the footnote getting in the way
\pagebreak
...
\subsection{Part 5: Defining the Parameter Comparison}
...
(\pyv{pm.summary(trace)}). Below\ \ref{singlepatientdiagnosticmetrics} we can see the output from this methods:
...
The most important metrics here to keep an eye out is \(\hat{R}\) (\pyv{r_hat}), that is short for potential scale
